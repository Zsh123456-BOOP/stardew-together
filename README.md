...
**下一阶段目标：DeepSeek 控制玩家本体与一个同行 NPC，自主经营、正常睡觉过夜，推进完整游戏与全部成就。** 现已完成[双角色自主通关调研与实施方案](docs/双角色自主通关调研与实施方案.md)；该目标尚未实现，以下仍是 0.6 的真实能力范围。

**当前可运行阶段版为 C# Mod「同行 · Together 0.6」**：游戏内 F8 聊天、人设、自主生活、共同经营和记忆；内置共同手册与持久化共同心愿：按原生配方展开材料依赖，未解锁也可先备料，逐项商量分工，伙伴按真实目标采集并核对库存进度。百科问答、普通聊天、自主决策共享这些事实。不使用视觉、BM25 或向量数据库，正式游玩不依赖 Python 服务。
...
双击生成目录 `outputs/同行体验版/启动同行.command`。模型 Key 读取项目 `.env`，不进入 Git 或安装 ZIP。`Together-0.6.0-own-code.zip` 仅含新 Mod，需要单独准备 Squad 适配依赖；本机体验目录已配好。
