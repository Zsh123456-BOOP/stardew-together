...
public sealed partial class CompanionControl {
    private sealed record Region(GameLocation Location,int Objects,int Features,DateTime Until,HashSet<Point> Tiles,Dictionary<Point,int> Distances);
    ...
    private HashSet<Point> ReachableRegion(ISquadMate mate) {
        ...
        var distances=new Dictionary<Point,int>{{origin,0}};
        ...
        while(pending.Count>0 && checkedTiles.Count<Math.Min(16384,width*height)) {
            ...
            foreach(var d in offsets) {
                ...
                reachable.Add(next);distances[next]=distances[tile]+1;pending.Enqueue(next);
            }
        }
        regions[id]=new(location,location.objects.Count(),location.terrainFeatures.Count(),DateTime.UtcNow.AddSeconds(2),reachable,distances);return reachable;
    }
    private Point? StandingSpot(ISquadMate mate,Point point) {
        ...
        return neighbors.Where(p=>region.Contains(p) && AStarPathfinder.IsTilePassableForFollower(mate.Npc.currentLocation,p,mate.Npc))
            .OrderBy(p=>regions[Id(mate)].Distances.GetValueOrDefault(p,int.MaxValue)).Select(p=>(Point?)p).FirstOrDefault();
    }
}
