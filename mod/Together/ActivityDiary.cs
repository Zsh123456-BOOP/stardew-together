...
public sealed class ActivityDiary {
    ...
    public void Native(JsonElement action,int day,int time) {
        ...
        if(before.ValueKind==JsonValueKind.Object&&after.ValueKind==JsonValueKind.Object) {
            ...
            foreach(var key in a.Keys.Union(b.Keys)) {int delta=b.GetValueOrDefault(key)-a.GetValueOrDefault(key);if(delta!=0)Add(id+":bag:"+key,day,time,delta>0?"行动期间入包":"行动期间取用",key,location,Math.Abs(delta));}
            int cash=Num(after,"money")-Num(before,"money");if(cash!=0)Add(id+":cash",day,time,skill=="player.sleep"?"过夜现金变化":"现金变化","金币",location,cash);
        }
    }
