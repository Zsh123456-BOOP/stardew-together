...
public static class AgentToolDiscovery {
    ...
    public static Dictionary<string,string> Core(IReadOnlyDictionary<string,string> catalog) {
        ...
        if(result.ContainsKey("work.run"))result["work.run"]="常用模式完整参数：{goal:cleanup|water|harvest|forage|wood|stone|fiber|fish|store|refill,actor_id?:player或真实伙伴ID,location?:真实地图名,cleanup_id?:已批准清理单ID,count?:0..999,stock_target?:1..9999,item?:QID,include_trees?:bool,required_free_slots?:0..12,reserve_stamina?:15..270,until?:HHMM<=2300,max_food?:0..10}。stock_target表示全队目标库存；count始终为新增数量。经营材料count=0或省略时补齐已批准项目缺口，不能无用途囤料。cleanup+cleanup_id可交给玩家或伙伴，按既定片区混合清理（伙伴不砍整树），无需材料用途，不越过保护区。water/harvest/forage的count=0完成范围内目标；fish的count=1..100是实际新增捕获数(默认3)，省略item为任意鱼。程序持续选点、原生操作、拾取、补给并回原地点，无需坐标。fish/refill/完整伐木限玩家；NPC木材仅树枝。store按required_free_slots准备容量，不出售；存取货收尾可指定until至2500。默认保留20体力，22点停止。播种必须先farm.plan(seed,count)取得plan_id，再work.run(goal:plant,plan_id)，自动清障、翻土、播种和浇水，不能以harvest代替播种。其他模式mine_trip/加工/畜牧等先用tools.lookup(names:[work.run])查询完整定义。";
        return result;
    }
