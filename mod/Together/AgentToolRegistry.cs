...
public sealed class AgentToolRegistry {
    ...
    public static readonly Dictionary<string,string> Catalog=new(){
        ["context.read"]="{section:assets|labor_budget|farm_cleanup|service_hours|inventory_plan|companions|progression|business|day|schedule|earlier_observation_summaries|recent|memory|goals|operating_candidates|sleep_review|plan|task_card|prerequisites}: 按上下文省略指针补读完整字段；只读，结果可经query.read续读。",
        ...
        ["goal.create"]="placed的count是地点内目标总量，已有设施先看assets；同一未完成目标换request_id重试仍复用原目标，不重复预留；只有明确追加数量才指定additional:true。{request_id:string,entity:物品ID或craft:配方名或cook:菜名,count?:int,quality?:0|1|2|4,allow_new_facilities?:bool,additional?:bool,completion?:owned|crafted|cooked|placed,purpose?:string,run?:bool}: 幂等创建持久目标；建议run:true一轮创建并启动，原生配方展开依赖、整备并续作。返回的Id不同于request_id，后续暂停/查询只使用返回Id，不猜",
