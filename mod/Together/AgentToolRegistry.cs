...
public sealed class AgentToolRegistry {
    ...
    public static readonly Dictionary<string,string> Catalog=new(){
        ...
        ["day.routine"]="{enabled?:bool,assignments?:{orchard?:player,mail?:player,cooking_tv?:player,water?:角色ID,harvest?:角色ID,feed?:角色ID,pet?:角色ID,milk?:player,shear?:player,animal_collect?:player}}: 保存跨日农务分工；每晨按真实缺项生成独立角色队列，无需模型重复派同样农活，首次默认未启用",
    ...
    private object ExecuteCore(string tool,JsonElement args) {
        ...
        return tool switch {
            ...
            "orchard.read"=>PlayerExecutor.ReadOrchard(),"arcade.read"=>PlayerExecutor.ReadArcade(),
