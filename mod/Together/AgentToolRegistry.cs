...
public sealed class AgentToolRegistry {
    ...
    public static readonly Dictionary<string,string> Catalog=new(){
        ...
        ["farm.cleanup"]="{request_id:string,mode?:run|pause,scopes?:[roads|courtyard|fields|general|all|zone:分区ID],reserve_stamina?:0..270,until?:HHMM<=2200,daily_limit?:0..120,remove_trees?:bool}: 保存持久清理目标，自动分批选目标、工具、寻路、存货续做；默认清杂草/树枝/小石头，树木须分区allow_trees且remove_trees。默认不设目标数上限，18点停止；体力预算由模型指定；程序告知待办农务消耗，不叠加隐藏预留；daily_limit仅用于明确指定的每日目标上限，0不设。为播种清地应直接使用farm.plan→work.run plant，不另建竞争清理单。余量次日继续。相同request_id必须同参数，不重复创建；pause保留进度。AI暂停时不执行。当前执行角色为玩家，伙伴仍可独立承担已有农务/资源任务。",
        ...
        ["player.fish"]="{item?:指定鱼QID,count?:1..20,reserve_stamina?:0..270}: 在当前可钓地图自动寻找可达岸边，真实蓄力/抛竿/咬钩/反馈控杆/收鱼；按Farmer原生捕获计数核验，次数/背包/体力/时间受限，特殊奖励菜单需继续处理；指定item只选择条件合格水域，count仍按实际所有捕获计数；定向收集用work.run(goal=fish,item,count)",
        ["player.collect_home_gifts"]="{}: 自动回到自己的住宅，读取真实礼包位置，选择可达相邻站位，原生领取一份礼包并核验入包；没有礼包时返回完成数量0。不读取普通箱子，不赠送物品，不需要坐标。经营模式自动处理。",
        ...
        ["goal.run"]="{id:共同目标ID,mode?:run|pause}: 持续按依赖采集/取料/制作/烹饪/加工；无需模型重复下发子步骤。失败保留目标，相关事实变化才重算并返回阻碍，pause保留目标且取消其待执行任务，正在执行需action.cancel",
        ["goal.create"]="{request_id:string,entity:物品ID或craft:配方名或cook:菜名,count?:int,quality?:0|1|2|4,allow_new_facilities?:bool,completion?:owned|crafted|cooked|placed,purpose?:string,run?:bool}: 幂等创建持久目标；建议run:true一轮创建并启动，原生配方展开依赖、整备并续作。返回的Id不同于request_id，后续暂停/查询只使用返回Id，不猜",
