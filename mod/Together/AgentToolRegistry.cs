...
public sealed class AgentToolRegistry {
    ...
    public static readonly Dictionary<string,string> Catalog=new(){
        ...
        ["player.mine_access"]="{mode:enter|leave|elevator,level?:int}: 自动走到真实入口/返程梯/电梯并通过原生菜单换层；电梯仅已解锁5层倍数，普通入口到1层",
        ["player.bundle"]="{bundle:原生献祭ID,budget?:int,keep_gold?:int}: 自动到社区中心实际纸条、打开指定收集包，按品质和原生数量提交齐全的材料；金库需显式预算，普通献祭预算0。缺项返回实际缺口",
        ...
        ["player.service"]="{location:string,service?:shop|build|upgrade_house|upgrade_tools|animals|geodes|claim_tool|museum_donate|museum_reward,shop?:商店ID}: 沿真实路线前往并查找地图服务柜台，原生交互/普通对话/选择已指定服务；shop需shop ID。核验实际商店/服务菜单打开，claim_tool核验原生归还。只打开服务不表示已完成购买或建造",
        ...
        ["farm.plan"]="{seed?:物品ID,count?:1..96,max_daily_manual_water?:0..96,require_scarecrow?:bool,priority?:income|collection|low_labor}: 在农场/温室按已有种子和真实可达地形生成地块方案，保护出入口、工作站位，架子作物检查种下后可达性；返回plan_id，work.run(goal=plant,plan_id=...)自动翻土播种浇水补水。读取已施肥料/职业/临水水稻与跨季生长条件，返回最多3个排序方案和收获/次日现金预测；当前不采购种子、不优化机器加工，不把预测当实收。",
        ...
        ["day.routine"]="{enabled?:bool,assignments?:{water?:角色ID,harvest?:角色ID,feed?:角色ID,pet?:角色ID,milk?:player,shear?:player,animal_collect?:player}}: 保存跨日农务分工；每晨按真实缺项生成独立角色队列，无需模型重复派同样农活，首次默认未启用",
    ...
    private object ExecuteCore(string tool,JsonElement args) {
        ...
        return tool switch {
            ...
            "farm.economy"=>mod.PlanFarmEconomy(args),"farm.economy_status"=>mod.ReadFarmEconomy(args),"farm.execute"=>mod.ExecuteFarmEconomy(args),"farm.plan"=>mod.PlanFarm(args),
            ...
            "construction.read"=>PlayerExecutor.ReadConstruction(args),"shop.read"=>mod.ObserveShop(args),
