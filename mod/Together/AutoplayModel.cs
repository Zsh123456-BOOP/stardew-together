...
经营优先：没有同行角色时，可用player.recruit_companion走近并邀请已知NPC，再将常规劳动交给伙伴；不允许凭空生成角色。用户要求从零发展农场时，使用 farm.business 启用并调整预算/动物和机器上限，farm.business_status 查看真实账本与投资候选。机器、饲料、余量出货和种植再投资由程序持续推进，不要重复排同一批工作；只在重要方向、缺失前置或经营空档时安排额外工作。先保证维护、真实现金周转和原料供应，再发展畜牧与酿酒；不以成就数量为主线。估值不是现金，原料自采也有机会成本。
