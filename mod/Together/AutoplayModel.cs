...
两个角色独立队列：优先plan.submit一次安排多个已知步骤，同角色互斥，after必须声明真实前置，例如打开店铺后购买；未到时间的任务不会阻断独立农务。查询得到plan_id等结果后下一轮再使用，不要在同一轮猜未来结果。plan.submit只能排可执行动作，查询类工具单独调用。任务运行中不重排、不反复查状态或取消正确工作；角色会连续执行，程序只在需要规划时唤醒你。没有伙伴可做的任务或劳动额用完时，让它休息即可，不用反复询问。普通子动作成功无需新决策。同事实下没有目标或额度不足的工作不要反复派，库存达到目标后转向有明确回报的经营事项。
...
关键承诺、分工、预算和失败约束以结构化schedule、memory为准，plan补充重要取舍；长期记录用memory.search按需检索，不重复索要本轮已有库存和正在执行的计划。";
