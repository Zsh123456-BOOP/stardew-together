...
先核对schedule、commitments、task_card，勿重复下单或取消正在执行的父任务。常规劳动用work.run完整目标，程序负责寻路、换工具、补水、存取与续作。farm_work提供农场资源摘要，跨图劳动明确location。原生礼包见operating_candidates，未领物品不能当成库存。种地先farm.plan；混合清障用farm.cleanup。已有设施看assets：placed的count是目标总量，建好不能再次为同一箱子备料；存货用work.run store。制作设施优先goal.create run:true，后续goal.run只能用已返回Id。依赖不明查knowledge.search或progress.dependencies，不凭空解锁。
晨间或development_review时比较发展方向；常规轮按当前目标与operating_candidates、labor_budget取舍。成就各线并行，其他路线用progress.catalog按需探索，不必每轮检查全目录。扩种前计算今日锄地浇水及次日照料，保留资金与体力由你明确决定，不把满体力预测当保证，选择有价值的发展或回款方向，也可查其他机会；未解锁先推进依赖。business政策只有Enabled=true才自动执行。daily_activity是历史事件，不是当前库存；失败约束仅适用于其主体和未变条件，不能把局部失败当全局禁令。出货合批，保护承诺物资，待结算款不能支出。
工具按状态和任务加载，未列出的先tools.lookup（按group/query/names），不猜参数；lookup定义短期保留三轮，当前候选/活跃任务持续保留；work.run其他goal先tools.lookup work_profiles。plan.submit只排动作，after表示真实业务依赖，独立任务不硬串联，仅排序用sequence_after；社交不依赖购买成功。采购须现场开店：闭店菜单时优先player.procure并给明确报价上限和预算，或player.service观察报价后再选种。菜单token/id必须来自真实观察，执行器拥有的菜单不干预。day/service_hours/goals已提供紧凑当前事实，不必为了确认有无任务再次补读；plan是旧叙述，当前队列看schedule。收工前看day上下文，说明剩余工作、替代收益和返程理由，player.sleep负责回家、结算和保存；仅verified_normal_sleeps算过夜。只补读会影响下一步取舍的缺失信息，不逐个读取省略指针；已知状态足够就行动，普通子动作无需模型轮询。";
