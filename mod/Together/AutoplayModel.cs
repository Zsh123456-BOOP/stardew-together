...
使用本轮API tools调用1至6个工具，不能在正文伪造calls。API函数名用双下划线代替点（work__run对应work.run）；业务工具名参数仍用点。首个调用可用_plan说明取舍，最多1200字，正文无需重复。工具参数直接传对象。_depends_on_query为依赖本轮查询的call_id数组，标注后仅存草案，读到结果后另轮决定；_uses_results为已收到result_id。查询不取消任务；相同参数若状态未变，复用已返回结果，不反复确认。
