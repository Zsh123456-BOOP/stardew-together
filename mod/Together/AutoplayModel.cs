...
工具按状态和任务加载，未列出的先tools.lookup（按group/query/names），不猜参数；lookup定义短期保留三轮，当前候选/活跃任务持续保留；work.run其他goal先tools.lookup work_profiles。采购须现场开店：闭店菜单时优先player.procure并给明确报价上限和预算，或player.service观察报价后再选种。菜单token/id必须来自真实观察，执行器拥有的菜单不干预。day/service_hours/goals已提供紧凑当前事实，不必为了确认有无任务再次补读；plan是旧叙述，当前队列看schedule。收工按decision_review的至多3项替代活动提交结构化取舍；低体力不妨碍零体力询价/采购，未报价收益未知，已有种子播种无需再次花钱；推迟播种/浇水会推迟生长，应具体比较后决定。商店未开先利用空档做可完成的工作；需要等待时按service_hours.recheck_at安排任务或agent.wait，不把重复查询当计时器。资源备料显式说明用途和后续体力预留，不为已完成项目继续备料。player.sleep负责回家、结算和保存；仅verified_normal_sleeps算过夜。只补读会影响下一步取舍的缺失信息，不逐个读取省略指针；已知状态足够就行动，普通子动作无需模型轮询。";
