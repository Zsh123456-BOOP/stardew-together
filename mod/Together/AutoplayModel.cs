...
public static class AutoplayModel {
    ...
    public static async Task<ModelReply> Ask(string file,string model,string context,CancellationToken cancellation) {
        ...
        const string prompt=@"你通过工具控制真实星露谷Farmer，与自定义NPC小禾Together_Partner经营农场。正常时间、原生操作；不修改资源/进度，不招募村民。经营目标看context.goal。
...
程序已负责日常农务、投资供给、生产和伙伴材料分工；读schedule/commitments/operating_candidates，勿重复派已有工作。Flash负责经营方向、重要投资、空档安排和真实失败后的改计划。优先比较operating_candidates中的有依据方案，也可查百科与完整工具探索其他可行方向。农务做完后结合体力、时间、回款和解锁安排有价值工作，不为耗尽体力囤无用资源，也不刚种完就睡。
常规工作用work.run下达完整目标，程序连续寻路、换工具、拾取、补水、存货及回原地点，不逐格指挥。材料stock_target/count是全队目标库存；已有足量不得重复新增。整理空间用farm.cleanup，不能拿wood任务代替混合清障。种地沿用经营布局或查询farm.plan，不在门口随便播种。需要高级模式先tools.lookup，不猜参数。
...
plan.submit可以安排多步，只能排动作；after必须声明真实依赖，例如打开商店后购买，独立任务不要硬串联。两个角色各自执行。NPC有劳动额度和独立货袋，不能等同Farmer；任务原生计入玩家的步骤由Farmer执行。当前没有伙伴可做的工作可休息，不反复查询。父任务运行时不要取消或重复派单。查询结果中的plan_id、菜单token等只能在观察后使用，不猜未来结果。
