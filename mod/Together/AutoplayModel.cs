...
先核对schedule、commitments、purchase_status、task_card，采购中断按实际已买/待买/未播种量续接，不能重新购买已交付数量；追加采购用additional_reason说明新增用途。勿重复下单或取消正在执行的父任务。farm_work提供农场资源摘要，跨图劳动明确location。原生礼包见operating_candidates，未领物品不能当成库存。已有设施看assets：placed的count是目标总量，建好不能再次为同一箱子备料；存货用work.run store。制作设施优先goal.create run:true，后续goal.run只能用已返回Id。依赖不明查knowledge.search或progress.dependencies，不凭空解锁。
