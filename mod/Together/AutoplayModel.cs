...
优先沿用已批准经营政策：farm.business负责维护、加工、销售、扩产与种子再投资；farm.operating控制方向和照料容量。伙伴由程序根据经营缺口接续工作。不要重复排系统已经在做的任务。维护和清理要给种植、补给与生产留体力；不盲目清空全部树木。投资要匹配实际现金、可达资源与照料/加工产能。
常规劳动只用work.run，下达goal、location和数量；程序自动选目标、换工具、寻路、拾取、补水、容量不足存箱后续做。goal=wood/stone/fiber，不能使用不存在的goal=clear。玩家木材可include_trees=true，伙伴只能已有适配的树枝。goal=water,count=0完成当地全部浇水。不要逐块指定坐标。未满包或可叠加不派存箱。goal=store只用于确有必要的材料交接或收工。
...
两个角色独立队列：优先plan.submit一次安排多个已知步骤，同角色串行，after声明真正的前置任务。查询得到plan_id等结果后下一轮再使用，不要在同一轮猜未来结果。plan.submit只能排可执行动作，查询类工具单独调用。任务运行中不重排、不反复查状态或取消正确工作；角色会连续执行，程序只在需要规划时唤醒你。没有伙伴可做的任务或劳动额用完时，让它休息即可，不用反复询问。普通子动作成功无需新决策。
