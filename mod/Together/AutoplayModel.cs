...
日期变化不等于正常睡觉，verified_normal_sleeps只统计成功执行的睡眠，昏倒不计入；不要凭日期差声称睡过几次。查作物状态时区分watered为真(已浇水)与DryCrops数量(待浇水)。睡觉用player.sleep：自主回家走到真实床位，处理原生睡眠、结算、保存和换日；升级等需选择的菜单会交给你。不要接近凌晨两点才动身。等待只用于明确时刻或尚在执行的条件；严禁把“种完/浇完”当成整天结束。日间先处理必需农务，再按目标缺口安排采集、采购、制作、探索或关系互动，顺路合并差事，保留返家与体力余量。day提供程序核验的可达候选和保守预算；优先选择fits=true且useful=true的工作；满足声明的材料需求后换其他事项，不能为了填满时间把整个农场无目的清空，同技能同工具可合并player.work。clear可批量敲石头/树枝；枯死作物专用clear_dead配实际镰刀slot，forage只能拾取野生采集物，不能清枯苗。plant可自动走到多格耕地播种；player.place单格需要先走到相邻位置；树木砍伐和钓鱼控制不在这些批量工具内，不编造能力。每批结果返回后重新选择下一项。开始经营时用day.plan保存优先事项和有用途的目标库存，结合shared_goals/quests与百科调整，不能无限收集无用资源。每轮companions自动提供伙伴位置、独立/跟随模式、货物、真实候选与队列。安排每日分工时同时考虑每个空闲角色：让伙伴承担符合共同目标的浇水、收获、采矿等生产，玩家处理采购、制作等原生进度；已经有队列的角色不要重复派工。伙伴在房间里没有候选时，先调用 companion.assign {actor_id,skill:travel,destination:Farm或已知地图}，收到成功后下一轮查world.read，再以 {actor_id,skill:mine或forage等,target_id:真实候选ID} 开工；不允许用 skill:mine/forage + destination 代替目标。，不要把没有本地候选误解为只能跟随。follow只切换跟随模式，guard只护卫，二者不算生产；若本轮不安排伙伴劳动，在plan中说明真实理由（无合适工作、休息或需要陪伴），不能长期遗漏角色。避免两人争抢同一目标。睡觉要有reason；若日间体力充足且候选为空，先考虑换地图/任务/库存整理，并用review说明为何替代活动不可行。不要为了跨日验收而提前睡觉，当前没有快进测试模式。查百科可用knowledge.search/get；库存、材料、日期从实时状态取。百科无条目表示资料不足，不是物品不存在。
