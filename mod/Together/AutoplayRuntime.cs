...
public sealed partial class ModEntry {
    ...
    public void StartAutoplay(string goal) {
        ...
        Data.Business.Enabled=true; // Autonomous control already authorizes ordinary business tools.
    ...
    private void TickAutoplay() {
        if(Context.IsWorldReady&&AutoplayRunning&&Data.Autoplay.TrialTargetDay>=0&&Game1.Date.TotalDays>=Data.Autoplay.TrialTargetDay&&Data.Autoplay.SleepDays>=Data.Autoplay.TrialTargetSleeps&&Game1.activeClickableMenu==null&&!Game1.eventUp&&!Game1.fadeToBlack) {
            ...
            PauseAutoplay("已完成指定自主试玩天数，保留窗口与日志");Persist();return;
        }
        ...
        if(decisionBlockedReason!=null) {
            // Finish already admitted native work before the explicit fault hold.
            if(!playerExecutor.Busy&&!WorkActorBusy("player")&&!Data.Autoplay.Schedule.Tasks.Any(t=>t.state=="running"))PauseAutoplay(decisionBlockedReason);
            return;
        }
        ...
        // Retain read results until the admitted work finishes or fails; only
        // native input menus and danger interrupt its execution.
