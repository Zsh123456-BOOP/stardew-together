...
public sealed partial class ModEntry {
    ...
    private object ReadBusinessLedger() {
        ...
        return new{commitments=OperationCommitments(),policy=Data.Business,operating=ReadOperatingLedger(),cleanup=FarmMaintenanceSummary(),cash=p.Money,total_earned=p.totalMoneyEarned,energy=new{current=p.Stamina,base_reserve=DailyBudget.EnergyReserve,pending_farm=PendingFarmEnergy(),available_new_planting=AvailablePlantingEnergy()},
            ...
            farm_investment=Data.FarmInvestment,tasks=Data.Autoplay.Schedule.Tasks.Where(t=>!t.Terminal).Select(t=>new{t.spec.id,t.spec.intent_id,t.spec.source,t.spec.actor,t.spec.tool,t.state,t.spec.purpose,t.spec.not_before,t.wait_reason,t.error}),
