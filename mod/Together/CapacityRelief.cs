...
public static class CapacityRelief {
    public static CapacityReliefVerdict Choose(CapacitySnapshot start,IReadOnlyList<CapacityCandidate> candidates,int depth,bool delivery=false) {
        ...
        foreach(var c in candidates) {
            ...
            if(verdict is {Feasible:true}&&verdict.FreeSlotsAtEnd<=start.FreeSlots&&!(delivery&&c.Id.StartsWith("store:")&&c.Operations.OfType<TakeOp>().Any(o=>o.Count>0)))reason="does_not_release_a_slot";
