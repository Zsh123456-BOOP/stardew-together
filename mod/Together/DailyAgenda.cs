...
public static class DailyBudget {
    ...
    public static int WorkMinutes(int time,int returnReserve)=>Math.Max(0,23*60-Minutes(time)-returnReserve);
    public static bool Fits(int time,float stamina,int reserve,int duration,float energy)=>WorkMinutes(time,reserve)>=duration && (energy<=0?stamina>=0:stamina-energy>=15);
    public static string? SleepBlock(int time,float stamina,bool freeWork,bool usefulWork,bool chores,bool reviewed,string reason) {
        ...
        if(stamina<=15 && !freeWork)return null;
