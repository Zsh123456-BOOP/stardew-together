...
public sealed class FarmBusiness {
    ...
    public bool Automation {get;set;} // Explicit model-created routine, not capability permission.
    public bool Expand {get;set;}=true;
    public int DailyBudget {get;set;}=-1;
...
public static class BusinessMath {
    ...
    public static int Spendable(int cash,int keep,int budget,int committed)=>Math.Max(0,Math.Min(cash-keep,budget<0?int.MaxValue:Math.Max(0,budget-committed)));
