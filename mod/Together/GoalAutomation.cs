...
public sealed partial class ModEntry {
    ...
    private void TickGoalAutomation() {
        ...
        if(Data.Autoplay.Schedule.Tasks.Any(t=>t.spec.actor=="player"&&!t.Terminal))return;
        foreach(var goal in Data.SharedGoals.Where(g=>g.AutoExecute&&g.Status=="active").OrderBy(g=>g.AutoReviewDay).ThenBy(g=>g.AutoReviewMinute).ToArray()) {
            try {
                ...
                if(goal.AutoBlockedConditions==condition&&goal.AutoReviewDay==Game1.Date.TotalDays&&minute<goal.AutoReviewMinute+60)continue;
