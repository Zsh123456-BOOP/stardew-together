...
public sealed class GoalNode {
    ...
    public int ToPrepare=>Math.Max(0,Missing-InProgress-Planned);
...
public sealed class GoalLedger {
    ...
    public GoalLedger Clone()=>new(stock);
    public void Replace(GoalLedger other){stock.Clear();stock.AddRange(other.Clone().stock);}
    public void Add(string item,int count)=>stock.Add(new(){Item=item,Count=count});
...
public static class GoalPlanner {
    ...
    public static void Rebuild(SharedGoal goal,IReadOnlyDictionary<string,GoalRecipe> recipes,GoalLedger ledger,int day,
        Func<string,string> name,int crafted=0,GoalLedger? processing=null,int placed=0) {
        if(goal.Status!="active"){goal.Reserved.Clear();return;}
        goal.Nodes=new();goal.Reserved=new();
        int budget=96;
        var future=new List<(string Item,int Count,string Producer)>();
        string Expand(string item,int count,string path,HashSet<string> ancestors,int depth,GoalRecipe? selected=null,int quality=0) {
            ...
            bool placementEvidence=path=="root"&&goal.Completion=="placed";
            node.Owned=placementEvidence?Math.Min(count,placed):craftingEvidence?Math.Min(count,Math.Max(0,crafted-goal.BaselineCrafts)):ledger.Take(item,count,quality);
            if(node.Owned>0&&!craftingEvidence&&!placementEvidence)goal.Reserved.Add(new(){Item=item,Count=node.Owned,Quality=quality,Name=node.Name});
            if(node.Missing==0){node.Status="ready";node.Reason="这份目标已分配到足量库存";return path;}
            if(placementEvidence){node.Kind="place";node.Status="waiting";node.DependsOn.Add(Expand(item,node.Missing,path+"/item",new(ancestors),depth+1,selected));if(goal.Nodes.First(n=>n.Id==node.DependsOn[0]).Status=="ready")node.Status="player_step";return path;}
            ...
            for(int i=0;i<future.Count&&quality==0&&node.ToPrepare>0;i++) {
                var entry=future[i];if(entry.Item!=item)continue;int take=Math.Min(entry.Count,node.ToPrepare);if(take<=0)continue;
                node.Planned+=take;node.DependsOn.Add(entry.Producer);future[i]=(entry.Item,entry.Count-take,entry.Producer);
            }
            if(node.ToPrepare==0){node.Status="planned";node.Reason="复用已计划批次的余量；等待真实产出后才计入库存";return path;}
            ...
            var alternatives=recipes.Values.Where(r=>r.Item==item && (r.Kind!="process" || r.Known || depth==0 || goal.AllowNewFacilities)).ToArray();
            var recipe=selected??GoalRoutes.Select(alternatives,recipes,ledger,node.ToPrepare,goal.AllowNewFacilities);
            ...
            if(!recipe.Supported){node.Status="blocked";node.Reason=recipe.UnsupportedReason;return path;}
            node.Status=recipe.Known?"waiting":"locked";
            node.Reason=recipe.Kind=="process"?"备料后在"+name(recipe.Facility)+"加工；先收齐原料，最终以实际产物核验。"+(recipe.Known?"":"还没有观察到已放置的设备。"):(recipe.Known?"备料后执行原生制作":"配方尚未解锁；先备料。"+recipe.Unlock);
            ...
            if(recipe.Kind=="process" && !recipe.Known && recipe.Facility!="" && goal.AllowNewFacilities)node.DependsOn.Add(Expand(recipe.Facility,1,path+"/facility",new(ancestors),depth+1));
            int batches=(int)Math.Ceiling(node.ToPrepare/(double)Math.Max(1,recipe.Output));
            foreach(var input in recipe.Inputs.OrderBy(i=>GoalRoutes.IsCategory(i.Item)).ThenByDescending(i=>i.Quality)) {
            ...
            int surplus=batches*Math.Max(1,recipe.Output)-node.ToPrepare;if(node.Status!="blocked"&&surplus>0)future.Add((item,surplus,path));
            return path;
        }
        ...
        if(root.Missing==0 || craftedEnough) {
            goal.Status="fulfilled";goal.Reserved.Clear();goal.Summary=goal.Completion=="placed"?"原生场景对象确认设施已放置":craftedEnough?"原生制作计数确认目标数量已完成":"当前实际库存确认已拥有目标数量";
        } else {
        ...
        if(goal.Fingerprint!=fingerprint) {
            goal.Fingerprint=fingerprint;goal.Revision++;goal.History.Add(new(){Day=day,Text=goal.Summary});
