...
public sealed partial class PlayerExecutor {
    ...
    private void StartMineAccess(JsonElement args) {
        ...
        if(mineAccessMode is not ("enter" or "skull" or "leave" or "elevator")||mineAccessLevel is <0 or >120)throw new InvalidOperationException("invalid_mine_access");
        ...
        if(mineAccessMode=="skull"&&!Game1.player.hasSkullKey&&!Utility.IsPassiveFestivalDay("DesertFestival"))throw new InvalidOperationException("native_skull_key_required");
        destination=mineAccessMode=="skull"||mineAccessMode=="leave"&&Game1.currentLocation is MineShaft {mineLevel:>120}?"SkullCave":"Mine";
        if(mineAccessMode=="skull")mineAccessLevel=121;Current!.phase="mine_access_route";
    }
    private void TickMineAccess() {
        if(Game1.locationRequest!=null||Game1.fadeToBlack)return;
        if(Current!.phase=="mine_access_transition") {
            bool arrived=mineAccessMode=="leave"||mineAccessLevel==0?Game1.currentLocation.NameOrUniqueName==destination:Game1.currentLocation is MineShaft m&&m.mineLevel==mineAccessLevel;
        ...
        if(Game1.activeClickableMenu is DialogueBox {isQuestion:false} unlock&&mineAccessMode=="skull"&&Game1.currentLocation.Name=="SkullCave"&&Game1.player.hasUnlockedSkullDoor) {
            if(DateTime.UtcNow<nextInteraction)return;nextInteraction=DateTime.UtcNow.AddMilliseconds(400);unlock.finishTyping();unlock.receiveLeftClick(unlock.xPositionOnScreen+16,unlock.yPositionOnScreen+16);return;
        }
        if(Game1.activeClickableMenu is DialogueBox dialogue&&mineAccessMode=="leave") {
            ...
            if(Game1.currentLocation.lastQuestionKey!="ExitMine")throw new InvalidOperationException("native_mine_exit_question_changed");
            int index=Array.FindIndex(dialogue.responses,r=>r.responseKey is "Leave" or "Yes");if(index<0)throw new InvalidOperationException("native_leave_mine_option_missing");
        ...
        if(Game1.currentLocation is not MineShaft&&Game1.currentLocation.NameOrUniqueName!=destination){Travel();return;}
        ...
        if(mineAccessTile==null) {
            ...
            for(int y=0;y<l.Map.Layers[0].LayerHeight;y++)for(int x=0;x<l.Map.Layers[0].LayerWidth;x++) {
                ...
                bool match=mineAccessMode=="leave"?tile==115||action.FirstOrDefault()=="ExitMine":mineAccessMode=="elevator"?l is MineShaft?tile==112:action.FirstOrDefault()=="MineElevator":mineAccessMode=="skull"?action.FirstOrDefault()=="SkullDoor":action.FirstOrDefault() is "Mine" or "NextMineLevel";
        ...
        if(mineAccessMode=="enter"||mineAccessMode=="skull"&&Game1.activeClickableMenu==null){Current.phase="mine_access_transition";nextInteraction=DateTime.UtcNow.AddSeconds(20);}
    }
}
