...
public interface ICompanionControl {
    ...
    string ConfigureFarm(string json); NPC? GetCharacter(string name); bool ResumeDay(string name); bool AttachCustomCompanion(string name); bool SuspendCustomCompanion(string name);
}
...
public sealed partial class ModEntry:Mod {
    ...
    public override void Entry(IModHelper helper) {
        ...
        helper.Events.Display.RenderedHud+=(_,e)=>{
            ...
            string text=SinglePlayerMode?"单玩家经营 · "+AgentHud():$"{Settings.OpenKey} 同行 · 交谈对象 {Selected}  "+(AutoplayRunning || Data.Autoplay.Status=="paused" && Data.Autoplay.Goal.Length>0?AgentHud():Thinking?"正在想怎么回答你…":Current.Job?.Status is "active" or "waiting"?JobText(Current.Job):"聊聊 / 小约定");
