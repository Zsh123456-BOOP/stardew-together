...
public sealed class ModelRequestBudget {
    ...
    public static void Configure(string path,long limit,int bytesLimit) {
        lock(Gate)current=new(path,limit==0?0:Math.Clamp(limit,10000,100000000),Math.Clamp(bytesLimit,16000,1000000));
    }
    public static string Display() {
        lock(Gate) {
            if(current==null)return "尚未调用模型";
            try{var l=current.Read();return $"今日 {l.Requests} 次 · 已用 {l.Reported:N0} Token · 待核销 {l.Unconfirmed:N0} · {(current.limit==0?"总量不限额":"预算余量 "+Math.Max(0,current.limit-l.Charged).ToString("N0"))}";}
    ...
    public static object Status() {
        lock(Gate) {
            ...
            try{var l=current.Read();return new{configured=true,charged=l.Charged,reported_tokens=l.Reported,unconfirmed_reserved=l.Unconfirmed,requests=l.Requests,unlimited=current.limit==0,limit=current.limit,remaining=current.limit==0?(long?)null:Math.Max(0,current.limit-l.Charged),request_bytes_limit=current.bytesLimit};}
    ...
    private void Reserve(long bound) {
        lock(Gate){var l=Read();if(limit>0&&bound>limit-l.Charged)throw new InvalidOperationException("daily_model_token_reservation_budget_exceeded");l.Charged+=bound;l.Unconfirmed+=bound;l.Requests++;Write(l);}
    }
