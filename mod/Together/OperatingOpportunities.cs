...
public sealed partial class ModEntry {
    private List<OperatingOpportunity> OperatingOpportunities() {
        ...
        if(player.Items.OfType<FishingRod>().Any()&&player.Stamina>=8&&Game1.timeOfDay<1900) {
            ...
            if(!blocked) {
                ...
                if(location.Length>0)rows.Add(new("fish-income","农务之外用可用体力获得真实渔获，补给后续作","work.run",new{goal="fish",location,count=3,reserve_stamina=0,until=Math.Min(2100,Game1.timeOfDay+400)},"rod_owned;native_fishing_conditions;catch_not_guaranteed",24,90));
            }
        }
