...
public sealed class OperatingPlan {
    ...
    public int PlayerWaterLimit {get;set;}=-1;
...
public static class OperatingMath {
    public static int CashForSeeds(int cash,int keep,int daily,int committed,int development)=>BusinessMath.Spendable(cash-Math.Max(0,development),keep,daily,committed);
