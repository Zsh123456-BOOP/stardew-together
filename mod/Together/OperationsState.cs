...
public static class OperationsPolicy {
    ...
    public static WorkOutcome Outcome(string tool,JsonElement result) {
        ...
        string disposition=status is "running" or "queued"?"in_progress":status=="waiting"?"waiting_condition":status=="partial"?(changed?"partial":"waiting_condition"):deferred?"deferred":status=="succeeded"?changed?"completed":navigation?"observed":"already_satisfied":reason is "remaining_targets_unreachable" or "no_matching_targets" or "no_eligible_targets_check_capability_path_or_cargo"?"no_candidates_found":RecoveryPolicy.CanWait(reason)?changed?"partial":"waiting_condition":status=="cancelled"?"cancelled":"failed";
