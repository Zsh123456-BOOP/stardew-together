...
public sealed partial class PlayerExecutor {
    private static Item? Equipped(string target)=>target switch{"left_ring"=>Game1.player.leftRing.Value,"right_ring"=>Game1.player.rightRing.Value,"boots"=>Game1.player.boots.Value,"hat"=>Game1.player.hat.Value,"shirt"=>Game1.player.shirtItem.Value,"pants"=>Game1.player.pantsItem.Value,"trinket"=>Game1.player.trinketItems.FirstOrDefault(),_=>throw new InvalidOperationException("unsupported_equipment_slot")};
    internal static object ReadEquipment()=>new{slots=new[]{"left_ring","right_ring","boots","hat","shirt","pants","trinket"}.Select(slot=>new{slot,item=AgentToolRegistry.ItemInfo(Equipped(slot)),unlocked=slot!="trinket"||Game1.player.stats.Get("trinketSlots")>0}),tools=Game1.player.Items.Select((item,slot)=>(item,slot)).Where(x=>x.item is Tool).Select(x=>new{slot=x.slot,item=x.item.QualifiedItemId,attachments=((Tool)x.item).attachments.Select((item,index)=>new{index,item=AgentToolRegistry.ItemInfo(item)})}),inventory=AgentToolRegistry.Inventory()};
    private void StartEquipment(JsonElement args) {
        ...
        if(targetSlot=="trinket"&&Game1.player.stats.Get("trinketSlots")==0)throw new InvalidOperationException("combat_mastery_trinket_slot_locked");
        ...
        if(remove) {
            ...
            selected=item;
            bool compatible=targetSlot switch{"left_ring" or "right_ring"=>item is Ring,"boots"=>item is Boots,"hat"=>item is Hat,"shirt"=>item is Clothing c&&c.clothesType.Value==Clothing.ClothesType.SHIRT,"pants"=>item is Clothing c&&c.clothesType.Value==Clothing.ClothesType.PANTS,"trinket"=>item is Trinket,_=>false};
        ...
        string nativeName=targetSlot switch{"left_ring"=>"Left Ring","right_ring"=>"Right Ring","boots"=>"Boots","hat"=>"Hat","shirt"=>"Shirt","trinket"=>"Trinket",_=>"Pants"};
