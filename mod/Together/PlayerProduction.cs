...
public sealed partial class PlayerExecutor {
    internal Item ValidateRecipe(CraftingRecipe recipe,bool cooking,string goal="") {
        if(!(cooking?Game1.player.cookingRecipes:Game1.player.craftingRecipes).ContainsKey(recipe.name))throw new InvalidOperationException("recipe_not_known");
        ...
        ValidateConsumption?.Invoke(consumption,goal,expected.QualifiedItemId);
    ...
    private void StartProduction(string skill,JsonElement args) {
        if(NativeMenuTools.HeldItem()!=null)throw new InvalidOperationException("production_output_pending_receive_before_new_recipe");
    ...
    private void TickProduction() {
        ...
        for(int page=0;page<productionMenu.pagesOfCraftingRecipes.Count;page++) {
            ...
            var expected=ValidateRecipe(recipe,Current.skill=="player.cook",productionGoal);
            ...
            Current.effects.Add(new{kind="native_recipe_order",recipe=productionRecipe,remaining_before=productionRemaining,inventory_before=inventoryBefore,inventory_after=inventoryAfter,held=productionMenu.heldItem==null?null:new{id=productionMenu.heldItem.QualifiedItemId,count=productionMenu.heldItem.Stack}});
