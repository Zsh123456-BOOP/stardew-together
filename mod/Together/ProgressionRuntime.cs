...
public sealed partial class ModEntry {
    ...
    private object DailyProgressDigest() {
        var p=Game1.player;
        var quests=p.questLog.Select(q=>new{id=NativeQuestIdentity.Id(q),title=q.questTitle,objective=q.currentObjective,complete=q.completed.Value,days_left=q.daysLeft.Value,description=q.questDescription}).ToArray();
        ...
        return new{day=Game1.Date.TotalDays,date=new{year=Game1.year,season=Game1.currentSeason,day=Game1.dayOfMonth},route=Facts.Route,
            active_quests=quests.Take(12),quest_total=quests.Length,unread_mail=Game1.mailbox.ToArray(),
    ...
    internal object AgentProgression() {
        ...
        return new {
            ...
            active_quests=p.questLog.Where(q=>!q.completed.Value).Select(q=>new{id=NativeQuestIdentity.Id(q),title=q.questTitle,description=q.questDescription}).Take(12).ToArray(),
            campaign=Data.Autoplay.Campaign,
