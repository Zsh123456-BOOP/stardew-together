...
public sealed partial class ModEntry {
    ...
    internal object SelectSeeds(JsonElement args) {
        ...
        if(chosen.Count==0)Data.FarmInvestment.Purchase.Close(Game1.Date.TotalDays,"selection_declined");
    ...
    private object SeedPurchaseStatus()=>new {
        day=Data.FarmInvestment.Purchase.Day,phase=Data.FarmInvestment.Day==Game1.Date.TotalDays?Data.FarmInvestment.Phase:"new_day",error=Data.FarmInvestment.Day==Game1.Date.TotalDays?Data.FarmInvestment.Error:"",plan_finalized=Data.FarmInvestment.Purchase.Day==Game1.Date.TotalDays&&Data.FarmInvestment.Purchase.PlanFinalized,plan_reason=Data.FarmInvestment.Purchase.Day==Game1.Date.TotalDays?Data.FarmInvestment.Purchase.PlanReason:"",
        items=Data.FarmInvestment.Purchase.Approved.Where(p=>Data.FarmInvestment.Purchase.Day==Game1.Date.TotalDays).Select(p=>new{item=p.Key,selection_upper_limit=p.Value,planned_total=Data.FarmInvestment.Purchase.PlanFinalized?(int?)Data.FarmInvestment.Purchase.Planned.GetValueOrDefault(p.Key):null,not_scheduled=Data.FarmInvestment.Purchase.NotScheduled(p.Key),purchased=Data.FarmInvestment.Purchase.Purchased.GetValueOrDefault(p.Key),remaining_to_buy=Data.FarmInvestment.Purchase.PlanFinalized?Data.FarmInvestment.Purchase.Remaining(p.Key):0,unplanted_owned=OwnedSeeds().Where(i=>i.QualifiedItemId==p.Key).Sum(i=>i.Stack)}).ToArray(),
        rule="selection_upper_limit仅选种上限；planned_total才是实际采购计划，not_scheduled为主动未安排数量，不是采购失败、不是待补买。remaining_to_buy只计已定计划尚未到货。purchased只据原生回执，未种库存另列；失败追加不会创建待办。done且remaining_to_buy=0表示该批已完成。"
    };
}
