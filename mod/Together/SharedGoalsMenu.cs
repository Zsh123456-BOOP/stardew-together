    ...
    public override void draw(SpriteBatch b) {
        ...
        if(Goal is {} goal) {
            ...
            int row=0;foreach(var node in goal.Nodes.Skip(offset).Take(PageSize)) {
                ...
                string status=node.Status switch {"ready"=>"已备齐","locked"=>"待解锁","player_step"=>"等你制作","waiting"=>"等材料","blocked"=>"需换途径",_=>"待准备"};
                Text(b,Fit($"{node.Name}：{node.Owned}/{node.Required} · {status} · {owner}",width-55),28,y);
