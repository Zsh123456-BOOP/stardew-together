...
public static class ToolSpecs {
    ...
    public static readonly HashSet<string> WorkFields=new("actor_id goal item quality plan_id cleanup_id goal_id target_level start_level region travel_budget keep_gold include_trees required_free_slots max_food location count stock_target reserve_stamina until additional exact_quality order_id objective quest_id labor_review purpose".Split(' '));
    ...
    public const string SleepDescription="原生返家、结算、保存。reason简述为何结束今天；defer列出decision_review.sleep_alternatives中决定推迟的全部ID（最多3项），明确接受推迟劳动/播种/照料的后果。成本由程序计算，无需填写估算和长篇论证。允许合理早睡，无固定时间；还有活动任务时先完成，排队不是过夜成功。";
    public static ToolSpec Sleep(string name) {
        var schema=new JsonObject{["type"]="object",["properties"]=new JsonObject{["reason"]=Field("string"),["defer"]=new JsonObject{["type"]="array",["maxItems"]=3,["items"]=Field("string")},["_plan"]=Field("string")},["required"]=new JsonArray("reason","defer")};
    ...
    public static ToolSpec Work(JsonElement context) {
        ...
        var descriptions=new List<string>{"持续原生劳动，自动寻路/选工具/补给/卸货/续作；无需坐标。actor_id默认player；location默认当前图，water默认Farm，跨图请明确。count默认0（资源另述），范围0..999；reserve_stamina为0..270，玩家备料省略时按今日待办照料自动预留，其他劳动默认0；max_food默认3，0..10，仅非预留食物；until默认2400，HHMM，存取货可至2500。0目标仅核验清空后完成；部分完成如实返回数量/stop_reason，条件不明报告阻碍。实际装不下才寻找Farm/畜舍output箱，按storage策略必要时建箱；不因空槽少就卸货，不丢弃物品，额外掉落保留位置续收。"};
        ...
        foreach(string profile in WorkProfiles.Keys.Where(profiles.Contains))switch(profile) {
            ...
            case "resource":Add("purpose","string","材料用途；程序自动核算默认照料预留");Add("stock_target","integer");Add("include_trees","boolean");Add("quality","integer");
                ...
                descriptions.Add("木/石/纤维/硬木/矿物count为本次新增；stock_target是全队总库存目标，含货袋，不与count混用。省略或count=0补项目缺口，无缺口新增20。玩家备料可填purpose说明用途；省略reserve_stamina自动保留今日待办照料体力。labor_review仅用于特殊分配：care=preserve不得低于待办照料，defer须写tradeoff。材料已足不再劳动；额外备料说明用途。resource需item，hardwood限玩家且受工具等级限制；木材默认可砍成熟无树液器普通树，include_trees=false禁整树。");break;
