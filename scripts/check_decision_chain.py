...
with (out/'game.log').open('w') as log:
 ...
 try:
  ...
  calls=[dict(tool='player.service',args=dict(location='ImpossibleLocation',service='shop',shop='SeedShop')),dict(tool='shop.read',args={})]
  r,ev,d=chain(calls,'failed-prerequisite',120)
  check('failed-chain-terminates',not d.get('continuation'),d)
  shops=[x['payload']['result'] for x in ev if x.get('kind')=='tool_result' and x['payload'].get('tool')=='shop.read']
  check('failed-action-skips-query',shops[-1].get('error')=='decision_dependency_failed_replan' and not any(x.get('error')=='native_shop_not_open' for x in shops),shops)
