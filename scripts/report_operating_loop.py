...
def summarize(root, run_id=None):
    ...
    def state(day):
        return days.setdefault(day,dict(day=day,cash_first=None,cash_last=None,actual_spend_gold=0,pending_shipping_estimate=None,native_saves=[],sleep=[],failures=[],failure_releases=[],delivery_delays=[],slow_frames=[],performance=None,quality=None,candidate_decisions=[]))
    for row,source in rows:
        ...
        if kind=='quality_bedtime':d['quality']={k:p.get(k) for k in ('SleepTime','ActualAwakeMinutes','TimeBreakdownMinutes','WallSecondsByState','WallDetailSeconds','TransportMinutes')};d['performance']=p.get('performance')
        if kind=='native_saved':d['native_saves'].append(dict(**event,evidence=p))
        ...
        if kind in ('shop_quote_observed','seed_selection','plant_native_outcomes','purchase_ledger','native_saved'):chain.append(dict(day=day,**event,kind=kind,evidence=p))
    ...
    return dict(scope='source-backed observations, not acceptance; fixture date jumps invalidate quality percentages',run_ids=sorted({r.get('run','') for r,_ in rows}),events=len(rows),event_counts=dict(Counter(r['kind'] for r,_ in rows)),days=[days[k] for k in sorted(days)],pending_observations=pending,economic_chain_evidence=chain,gates=dict(G3='unset_requires_completed_baseline_and_user_threshold',G4='manual_join_required_no_inferred_reinvestment',fourteen_day_acceptance=False))
