...
def stats(values):
 v=sorted(values)
 return dict(count=len(v),mean=sum(v)/len(v) if v else None,p95=v[int((len(v)-1)*.95)] if v else None,max=v[-1] if v else None)
