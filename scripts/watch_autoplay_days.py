...
scenario('agent_start',goal='独立控制玩家与Abigail持续经营这份农场，推进原生任务、制作与建设。读取百科和实际存档规划：照料作物、合理安排当季种植，按有用途的资源缺口分工采集、制作与采购，处理真实任务，充分利用白天但保留体力和返家时间。优先一次安排多个可确定步骤，让两人独立推进；遇到失败查状态并调整，不反复发同一错误任务。正常上床保存，次日接着经营，不修改日期、物资或完成标记。')
