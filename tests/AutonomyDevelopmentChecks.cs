...
public static class AutonomyDevelopmentChecks {
    public static void Run(Action<bool,string> check) {
        ...
        var tiredPortfolio=CropPortfolio.Plan(new(1,1,500,0,100,15,15,"income",new(0,0),bedGrid,new(),new(){bedSeed}){EnergyBudget=24});
        check(tiredPortfolio.FirstDayEnergy<=24&&tiredPortfolio.Plants.Count<15,"plant portfolio includes clearing, tilling and first watering within current energy");
        check(bedPortfolio.Plants.All(p=>bedPortfolio.PreparationTiles.Contains(p.Tile))&&bedPortfolio.PreparationTiles.Count==bed.Tiles.Count,"execution preserves the full cleared bed footprint, including unused access tiles");
        ...
        var scarceGrid=zoneGrid.Select(c=>c with{Plantable=c.Tile.X>=2&&c.Tile.X<6&&c.Tile.Y>=2&&c.Tile.Y<6,ClearCost=c.Tile.X>=2&&c.Tile.X<6&&c.Tile.Y>=2&&c.Tile.Y<6?1:0}).ToList();
        var raisedSeed=bedSeed with{Seed="raised",Owned=0,SalePrice=300,Trellis=true,Quote=new("raised","shop","Town",1,60,99,1),Growth=scarceGrid.ToDictionary(c=>c.Tile,c=>4)};
        var smallSeed=raisedSeed with{Seed="small",SalePrice=2,Trellis=false,Quote=new("small","shop","Town",1,20,99,1)};
        var sparse=CropPortfolio.Plan(new(1,1,100,100,0,16,16,"income",new(0,0),scarceGrid,new(),new(){raisedSeed,smallSeed}));
        check(sparse.PreparationTiles.Count>sparse.Plants.Count&&sparse.Plants.Count>0,"sparse mixed portfolio retains cleared unused aisles instead of assuming debris vanished");
        var future=scarceGrid.Select(c=>c with{Passable=c.Passable&&(c.ClearCost==0||sparse.PreparationTiles.Contains(c.Tile))}).ToList();
        var reachable=FarmLayout.ReachableAfter(future,new(0,0),sparse.Plants.Where(p=>p.Seed=="raised").Select(p=>p.Tile));
        check(sparse.Plants.All(p=>new[]{new FarmCell(p.Tile.X-1,p.Tile.Y),new FarmCell(p.Tile.X+1,p.Tile.Y),new FarmCell(p.Tile.X,p.Tile.Y-1),new FarmCell(p.Tile.X,p.Tile.Y+1)}.Any(reachable.Contains)),"all future crop interaction stands remain connected to the farm entrance");
