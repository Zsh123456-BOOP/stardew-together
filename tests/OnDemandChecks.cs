...
public static class OnDemandChecks {
    ...
    public static void Run(Action<bool,string> check) {
        ...
        check(view.GetProperty("service_hours").GetArrayLength()==2&&!view.GetProperty("memory").GetProperty("daily_activity").TryGetProperty("previous",out _)&&source.GetProperty("memory").GetProperty("daily_activity").GetProperty("previous").GetArrayLength()==1,"future service windows remain visible while previous-day diary moves to readback without modifying frozen archive");
